...
class Population {
    ...
    for (let i = 0; i < this.populationSize; i++) {
      ...
      console.log(
        `Generated individual ${
          i + 1
        }: Genes: ${individual.getGenes()}, Fitness: ${individual.getFitness(
          scoreTable
        )}`
      );
  ...
  generate() {
    this.population.forEach((individual, i) => {
      ...
      const partnerB = this.population[b];
      console.log(
        `Selected partners No.${a + 1} genes: ${partnerA.getGenes()} & No.${
          b + 1
        } genes: ${partnerB.getGenes()}`
      );
      const child = partnerA.crossover(partnerB);
      ...
      individual.genes = child.genes;
      console.log(
        `Generated Crossover & Mutation get genes: ${individual.getGenes()}, Fitness:${individual.getFitness(
          this.scoreTable
        )}`
      );
    });
...
const targetFitness = 43000; // Set your target fitness value here
...
while (genCount < generations) {
  ...
  if (
    !bestIndividual ||
    currentBest.getFitness(scoreTable) < bestIndividual.getFitness(scoreTable)
  ) {
    bestIndividual = currentBest;
    console.log(
      `Best fitness updated at generation ${genCount}, Fitness: ${bestIndividual.getFitness(
        scoreTable
  ...
  if (bestIndividual.getFitness(scoreTable) <= targetFitness) {
    console.log(
      `Target fitness(${targetFitness}) reached. Best Fitness: ${bestIndividual.getFitness(
        scoreTable
      )}, Genes: ${bestIndividual.getGenes()}`
    );
    break;
  ...
  console.log(
    `Generation ${genCount}, Best Fitness: ${bestIndividual.getFitness(
      scoreTable
    )}, Genes: ${bestIndividual.getGenes()}`
  );
  genCount++;
...
if (bestIndividual.getFitness(scoreTable) > targetFitness) {
  console.log(
    `Closest fitness(${targetFitness}): ${bestIndividual.getFitness(
      scoreTable
    )}, Genes: ${bestIndividual.getGenes()}.`
  );
}
